...
class KinectVideoPublisher(Node):
    def __init__(self):
        ...
        # Timers
        self.video_timer = self.create_timer(0.066, self.kinect_video)
        self.depth_timer = self.create_timer(0.066, self.kinect_depth)
        # Para convertir imagen
        self.bridge = CvBridge()
        # Flag para controlar el shutdown
        self.running = True
        # Crear ventana una sola vez
        cv2.namedWindow("Kinect Video (presiona Q para salir)", cv2.WINDOW_NORMAL)
    def kinect_video(self):
        ...
        # Eliminar esta línea para no mostrar el video
        # cv2.imshow("Kinect Video (presiona Q para salir)", array)
...
def listen_for_q(node):
    print("Presiona Q en la ventana de video para cerrar el nodo.")
    while rclpy.ok():
        key = cv2.waitKey(1000)
        if key == ord('q') or key == ord('Q'):
            node.get_logger().info("Tecla Q presionada. Cerrando nodo...")
            node.running = False
            rclpy.shutdown()
            break
...
def main(args=None):
    ...
    q_thread = threading.Thread(target=listen_for_q, args=(node,), daemon=True)
    q_thread.start()
